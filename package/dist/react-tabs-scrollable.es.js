...
const Tabs = (props) => {
  ...
  const scrollSelectedIntoView = (index = activeTab, animation = true, isClicked) => {
    ...
    const centerPointOfTabs = tabsContainerRects.clientWidth / 2 - tabRects.width / 2;
    if (!tabsContainerRects || !tabRects) {
      return;
    }
    const centerOfViewValue = mode === "scrollSelectedToCenter" || mode === "scrollSelectedToCenterFromView" ? tabsContainerRects.clientWidth / 2 - tabRects.width / 2 : 0;
    const endOfViewValue = mode === "scrollSelectedToEnd" ? tabsContainerRects.clientWidth - tabRects.width : 0;
    ...
    if (centerPointOfTabs > tabRects[start] + tabRects.width && isClicked && mode === "scrollSelectedToCenterFromView") {
      isClicked = false;
      const nextScrollStart = tabsContainerRects[scrollLeft] + (tabRects[start] - tabsContainerRects[start]) - additionalScrollValue;
      scroll(nextScrollStart, animationDuration || selectedAnimationDuration, animation);
    } else if (centerPointOfTabs < tabRects[start] - tabRects.width && isClicked && mode === "scrollSelectedToCenterFromView") {
      isClicked = false;
      const nextScrollStart = tabsContainerRects[scrollLeft] + (tabRects[end] - tabsContainerRects[end]) + additionalScrollValue;
      scroll(nextScrollStart, selectedAnimationDuration || animationDuration, animation);
    }
